...
module.exports = {
  addCard: async (req, res) => {
    ...
    if (req.body.authCode === undefined) {
      ...
      console.log("New card created:", newCard.toJSON());

      await newCard.addProfile(profileAssociated);
      ...
      for (const card of userCards) {
        ...
        if (currentCardNumber === req.body.cardNumber) {
          ...
          for (const profile of card.Profiles) {
            ...
            const decryptedAuthCode = await encryptDecrypt.decrypt(
              profile.authCode
            );
            if (decryptedAuthCode === req.body.authCode) {
              ...
              if (
                ...
                card.cardOwnerName === req.body.cardOwnerName.toUpperCase()
              ) {
  ...
  getCardById: async (req, res) => {
    ...
    for (const profile of profileAssociated) {
      ...
      if (profileUser.UserId === req.user.id) {
        ...
        const originalCardNumber = await encryptDecrypt.decrypt(
          card.cardNumber
        );
  ...
  payBill: async (req, res) => {
    ...
    for (const profileCardId of allProfileCardIds) {
      ...
      const currentUserCardNumber = await encryptDecrypt.decrypt(
  ...
  getStatementsYearMonth: async (req, res) => {
    ...
    for (const profileCardId of allProfileCardIds) {
      ...
      const currentCardNumber = await encryptDecrypt.decrypt(
      ...
      if (currentCardNumber === req.params.id) {
        const statements = await db.Transaction.findAll({
          where: {
            CardId: profileCardId.CardId,
            transactionDateTime: {
              // we are now fetching all the statements between the starting and endingDate
              [Op.gt]: startingDate,
              [Op.lte]: endingDate,
            },
          },
